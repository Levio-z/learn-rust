...
#### 应用来自邮件的补丁
https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html
...
一种非常简单的工作流会直接将工作合并进入 `master` 分支。 在这种情况下，`master` 分支包含的代码是基本稳定的。 当你完成某个特性分支的工作，或审核通过了其他人所贡献的工作时，你会将其合并进入 master 分支，之后将特性分支删除，如此反复。 如果我们的版本库包含类似 [包含若干特性分支的提交历史。](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#merwf_a) 的两个名称分别为 `ruby_client` 和 `php_client` 的分支，并且我们先合并 `ruby_client` 分支，之后合并 `php_client` 分支，那么提交历史最后会变成 [合并特性分支之后。](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#merwf_b) 的样子。

![](asserts/Pasted%20image%2020250731142023.png)
Figure 1. 包含若干特性分支的提交历史。
![](asserts/Pasted%20image%2020250731142100.png)
...
如果你的项目非常重要，你可能会使用两阶段合并循环。 在这种情况下，你会维护两个长期分支，分别是 `master` 和 `develop`，`master` 分支只会在一个非常稳定的版本发布时才会更新，而所有的新代码会首先整合进入 `develop` 分支。 你定期将这两个分支推送到公共版本库中。 每次需要合并新的特性分支时（[合并特性分支前。](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#merwf_c)），你都应该合并进入 `develop` 分支（[合并特性分支后。](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#merwf_d)）；当打标签发布的时候，你会将 `master` 分支快进到已经稳定的 `develop` 分支（[一次发布之后。](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#merwf_e)）。


![](asserts/Pasted%20image%2020250731142137.png)


Figure 3. 合并特性分支前。

![](asserts/Pasted%20image%2020250731142155.png)
Figure 4. 合并特性分支后。
![](asserts/Pasted%20image%2020250731142227.png)
Figure 5. 一次发布之后。
...
Git 项目包含四个长期分支：`master`、`next`，用于新工作的 `pu`（proposed updates）和用于维护性向后移植工作（maintenance backports）的 `maint` 分支。 贡献者的新工作会以类似之前所介绍的方式收入特性分支中（见 [管理复杂的一系列接收贡献的平行特性分支。](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#merwf_f)）。 之后对特性分支进行测试评估，检查其是否已经能够合并，或者仍需要更多工作。 安全的特性分支会被合并入 `next` 分支，之后该分支会被推送使得所有人都可以尝试整合到一起的特性。
![](asserts/Pasted%20image%2020250731142405.png)
...
![](asserts/Pasted%20image%2020250731142503.png)
...
![](asserts/Pasted%20image%2020250731142944.png)
...
![](asserts/Pasted%20image%2020250731143009.png)
...
如果你需要和 rerere 的缓存交互，你可以使用 `git rerere` 命令。 当单独调用它时，Git 会检查解决方案数据库，尝试寻找一个和当前任一冲突相关的匹配项并解决冲突（尽管当 `rerere.enabled` 被设置为 `true` 时会自动进行）。 它也有若干子命令，可用来查看记录项，删除特定解决方案和清除缓存全部内容等。 我们将在 [Rerere](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#_rerere) 中详细探讨。

#### 为发布打标签

当你决定进行一次发布时，你可能想要留下一个标签，这样在之后的任何一个提交点都可以重新创建该发布。 你在 [[_git_basics_chapter]](https://bingohuang.gitbooks.io/progit2/content/05-distributed-git/sections/maintaining.html#_git_basics_chapter) 中已经了解了创建新标签的过程。 作为一个维护者，如果你决定要为标签签名的话，打标签的过程应该是这样子的：
