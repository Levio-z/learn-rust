...
- **做法：** 将数据设为私有，仅通过函数（Getter/Setter）暴露。
- **收益：**
    - **监控点：** 你可以在 Setter 中**添加日志、断点或监控，瞬间定位是谁在什么时候修改了数据**。
    - **逻辑切入：** 你可以在修改值时加入校验逻辑（如：防止存入非法值）。
    - **逐步替换：** 有了函数封装，你可以更容易地将这个变量迁移到某个具体的对象中，而不需要修改数千处调用点。
