...
2. fork两次并杀死一级子进程，令二级子进程成为孤儿进程而被init所“收养”、清理[[../../../../../../Areas/Rust/Area/1 基本概念/2 进阶/2.1 所有权、生命周期和内存系统/2.1.3 生命周期和引用/引用机制/Rust-自动解引用/2 as_ref]](https://zh.wikipedia.org/wiki/僵尸进程#cite_note-2)。
...
- UNIX命令`ps`列出的进程的状态（"STAT"）栏标示为 "`Z`"则为僵尸进程。[[../../../../../../Areas/Rust/Area/1 基本概念/1 基础知识/RustBook/5.结构体/1.3 单元结构体]](https://zh.wikipedia.org/wiki/僵尸进程#cite_note-1)
