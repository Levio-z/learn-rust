### 架构
![](asserts/Pasted%20image%2020250603195556.png)
...
- 每个任务的**调用栈都会有一定的内存浪费（空闲），并都会有栈溢出风险。**
	- 递归就会有栈溢出的风险
![](asserts/Pasted%20image%2020250603195848.png)
无栈协程则将任务的信息同一保存成状态机，统一存放在堆上，由执行器通过更改指针来切换执行的任务。
调用栈仅每个执行器一个，一定程度上减少了内存浪费、降低栈溢出风险。
![](asserts/Pasted%20image%2020250603200256.png)
无栈协程你必须有一个堆。
传统的中断处理程序通常会以函数调用的形式，流程如下：
1. 保存用户上下文；
2. 通过标准的函数调用形式跳转到中断处理程序；
3. 恢复用户上下文并退出。
![](asserts/Pasted%20image%2020250603200515.png)
这里使用一种新的方式，思想与去年的一等奖作品FTL OS不谋而合，其伪代码分成两个部分，进入用户和进入内核，分别对应右上图和右下图。
![](asserts/Pasted%20image%2020250603200635.png)
![](asserts/Pasted%20image%2020250603200638.png)
...
![](asserts/Pasted%20image%2020250603200749.png)
...
- 分离了线程的信息和状态：
	...
	- 将私有的状态转移到异步循环中的局部变量。
- 既充分利用了Rust的所有权系统，又由于细粒度锁提高了线程信息访问的并发性。
### ![](asserts/Pasted%20image%2020250603201002.png)   网络协议栈
...
![](asserts/Pasted%20image%2020250603202017.png)
### 文件系统
...
![](asserts/Pasted%20image%2020250603202146.png)页帧结构的关系图。其中上图为逻辑关系图；下图为实际存储关系图，箭头代表引用。同个颜色（内容）的页帧只存在一份副本。
### 为了并发：网络设备抽象
网络设备分成三部分抽象，独立成trait：设备信息、发送队列、接收队列。
通过令牌管理每个发送/接收包的缓冲区所有权。
借助Rust的所有权系统，可以最细粒度地分割网络设备和缓冲区的所有权和I/O操作的粒度，达到高并发的效果。
![](asserts/Pasted%20image%2020250603202336.png)
### 为了并发：SD卡设备
...
![](asserts/Pasted%20image%2020250603202517.png)
### 其他功能简介：地址空间管理
按需分配的原则
- 指定地址与大小：按地址分配
