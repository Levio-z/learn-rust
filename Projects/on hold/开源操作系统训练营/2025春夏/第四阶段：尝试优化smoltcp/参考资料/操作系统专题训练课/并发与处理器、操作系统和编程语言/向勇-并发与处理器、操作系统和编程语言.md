...
![](asserts/Pasted%20image%2020250603165107.png)

![](asserts/Pasted%20image%2020250603165603.png)


![](asserts/Pasted%20image%2020250603165635.png)
![](asserts/Pasted%20image%2020250603165656.png)
...
![](asserts/Pasted%20image%2020250603171531.png)
- 系统调用就是把原来的系统调用切割，并不天然认为是事件响应的过程
![](asserts/Pasted%20image%2020250603171222.png)
![](asserts/Pasted%20image%2020250603171700.png)
操作系统的中断响应过程
![](asserts/Pasted%20image%2020250603171731.png)
...
![](asserts/Pasted%20image%2020250603171812.png)
- 观察是否有信号处理，返回用户态才处理
信号处理
- ⽤户注册的信号处理函数都是⽤户态的
	- 先构造堆栈，返回⽤户态去执⾏⾃定义信号处理函数
	- 再返回内核态继续被信号打断的返回⽤户态的动作。
![](asserts/Pasted%20image%2020250603184324.png)
#### 进程、线程和协程
##### Multitasking
![](asserts/Pasted%20image%2020250603184503.png)
##### User-level Thread
...
progress
![](asserts/Pasted%20image%2020250603185741.png)
### 异步操作系统
...
![](asserts/Pasted%20image%2020250603192213.png)
![](asserts/Pasted%20image%2020250603192305.png)
##### ⻋春池、蒋周奇：共享调度器
在操作系统层⾯提供协程⽀持

- 共享调度器直接将所使⽤的代码、任务池资源都共享到⽤户，⽤户运⾏和内核相同的代码。
- ⽤户进程与内核以相同的逻辑处理任务池中的任务
![](asserts/Pasted%20image%2020250603192358.png)
##### 王⽂智：线程与协程的统⼀调度 
...
4. 陷⼊函数退出。
![](asserts/Pasted%20image%2020250603193542.png)
##### 总结
